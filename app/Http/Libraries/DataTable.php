...
class DataTable{
    ...
    public static function get($where = array(), $joins = array(), $extraColumns = array(), $misc = array()) {
        // if(\request()->datatable['pagination']['start']){
        //     self::$_start = intval(\request()->datatable['pagination']['start']);
        // }
        // self::$_start = intval(\request()->datatable['pagination']['start']);
        self::$_length = intval(\request()->datatable['pagination']['perpage']);
        if (self::$_length <= 0) {
            self::$_length = 10;
        }

        if(\request()->datatable['pagination']['page'] > 1){
            self::$_start = (\request()->datatable['pagination']['page'] * self::$_length) - self::$_length;
        }else{
            self::$_start = 0;
        }
        ...
        self::$_draw = \request()->datatable['pagination']['page'];
